...
import { ArrowLeft, Calendar, Building2, Briefcase, ExternalLink, Volume2, MessageCircle } from "lucide-react";
...
const JobDetail = () => {
        ...
        <div className="uni-page-content">
          <div className="uni-container">
            <div style={{ textAlign: "center", padding: "60px 0" }}>
              <p style={{ color: "#6b7280", fontSize: 14, marginBottom: 16 }}>This job could not be found.</p>
              <Link to="/jobs" className="uni-btn-primary">Back to Jobs</Link>
            </div>
          </div>
        </div>
      </PublicLayout>
  ...
  const actualStatus = isExpired ? "Closed" : job.status;
  ...
  return (
    <PublicLayout>
      <div className="uni-page-header">
        <div className="uni-container">
          <div className="uni-breadcrumb">
            <Link to="/">Home</Link> / <Link to="/jobs">Jobs</Link> / {job.title}
          </div>
          <h1>{job.title}</h1>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 8, flexWrap: "wrap" }}>
            ...
            {job.experience_level && (
              <span style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 13, color: "rgba(255,255,255,0.7)" }}>
                <Briefcase size={14} /> {job.experience_level}
              </span>
            )}
            {job.deadline && (
              <span style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 13, color: "rgba(255,255,255,0.7)" }}>
      ...
      <div className="uni-page-content">
        <div className="uni-container">
          <div style={{ display: "flex", gap: 8, marginBottom: 24 }}>
            <Link to="/jobs" className="uni-btn-outline" style={{ fontSize: 13, padding: "6px 14px", gap: 6 }}>
            ...
            <div className="uni-detail-main">
              ...
              {job.description && (
                <div className="uni-detail-section">
                  <h2>Description</h2>
                  <div className="uni-rich-content" dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(job.description) }} />
                </div>
              )}
              ...
              {job.qualifications && (
                <div className="uni-detail-section">
                  ...
                  <div className="uni-rich-content" dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(job.qualifications) }} />
                </div>
              )}
            </div>
              ...
              <div className="uni-card">
                <div className="uni-card-body">
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                    <div>
                      <div style={{ fontSize: 11, fontWeight: 500, color: "#6b7280", textTransform: "uppercase", letterSpacing: "0.05em" }}>Company</div>
                      <div style={{ fontSize: 14, fontWeight: 500, color: "#111827", marginTop: 2 }}>{job.company}</div>
                    </div>
                    {job.experience_level && (
                      <div>
                        <div style={{ fontSize: 11, fontWeight: 500, color: "#6b7280", textTransform: "uppercase", letterSpacing: "0.05em" }}>Experience Level</div>
                        <div style={{ fontSize: 14, fontWeight: 500, color: "#111827", marginTop: 2 }}>{job.experience_level}</div>
                      </div>
                    )}
                    {job.deadline && (
                      <div>
                        <div style={{ fontSize: 11, fontWeight: 500, color: "#6b7280", textTransform: "uppercase", letterSpacing: "0.05em" }}>Deadline</div>
                        <div style={{ fontSize: 14, fontWeight: 500, color: "#111827", marginTop: 2 }}>{new Date(job.deadline).toLocaleDateString()}</div>
                      </div>
                    )}
                    <div>
